...
HERMES_AGENT_HELP_GUIDANCE = PRODUCT_HELP_GUIDANCE
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines = [
        "# Nous Subscription",
        "Nous subscription includes managed web tools (Firecrawl), image generation (FAL), OpenAI TTS, and OpenAI Whisper STT by default. Browser automation uses local CloakBrowser in k-hermes. Modal execution is optional.",
        "Current capability status:",
    ]
    lines.extend(_status_line(feature) for feature in features.items())
    lines.extend(
        [
            "When a Nous-managed feature is active, do not ask the user for Firecrawl, FAL, OpenAI TTS, or OpenAI Whisper API keys.",
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Return the agent identity for system-prompt slot #1.

    k-hermes product builds lock identity to the package constant so users
    cannot override persona via ``$HERMES_HOME/SOUL.md``. Upstream Hermes
    loads that file as personalization; Dolshoi does not.
    """
    try:
        from hermes_cli.product_identity import PRODUCT_IDENTITY_LOCKED
    except Exception:
