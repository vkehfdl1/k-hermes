...
class TestSystemPromptActiveProfile:
    def test_multi_profile_guidance_removed_from_system_prompt(self):
        """dolshoi keeps a single-profile product surface: the system prompt
        must not advertise multi-profile paths or cross-profile writes."""
        from pathlib import Path
        src = Path("agent/system_prompt.py").read_text()
        assert "Active Hermes profile" not in src
        assert "cross_profile=True" not in src
        assert "Single-profile product" in src
