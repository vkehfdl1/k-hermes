...
- **CloakBrowser 本地模式** — 通过 [CloakBrowser](https://github.com/jo-inc/cloakbrowser-browser) 实现本地反检测浏览（基于 Firefox 的指纹伪装）
...
[CloakBrowser](https://github.com/jo-inc/cloakbrowser-browser) 是一个自托管的 Node.js 服务器，封装了 CloakBrowser（一个带有 C++ 指纹伪装的 Firefox 分支）。它无需云端依赖即可提供本地反检测浏览。
...
或通过 `hermes tools` → Browser Automation → CloakBrowser 进行配置。

设置 `CLOAKBROWSER_URL` 后，所有浏览器工具将自动通过 CloakBrowser 路由，而非 Browserbase 或 agent-browser。

#### 持久化浏览器会话

默认情况下，每个 CloakBrowser 会话使用随机身份 — Cookie 和登录状态不会在 Agent 重启后保留。要启用持久化浏览器会话，请在 `~/.hermes/config.yaml` 中添加：
...
:::warning 嵌套路径很重要
Hermes 读取的是 `browser.cloakbrowser.managed_persistence`，**而非**顶层的 `managed_persistence`。常见错误写法：
...
##### Hermes 的行为
- 向 CloakBrowser 发送确定性的 profile 范围 `userId`，使服务器能够跨会话复用同一 Firefox profile。
...
##### Hermes 不做的事
- 不会强制 CloakBrowser 服务器持久化。Hermes 只发送稳定的 `userId`；服务器必须通过将该 `userId` 映射到持久化 Firefox profile 目录来支持它。
- 如果您的 CloakBrowser 服务器构建将每个请求视为临时的（例如始终调用 `browser.newContext()` 而不加载已存储的 profile），Hermes 无法使这些会话持久化。请确保运行的 CloakBrowser 版本实现了基于 userId 的 profile 持久化。

##### 验证是否正常工作

1. 启动 Hermes 和 CloakBrowser 服务器。
2. 在浏览器任务中打开 Google（或任意登录网站）并手动登录。
3. 正常结束浏览器任务。
4. 开始新的浏览器任务。
5. 再次打开同一网站 — 应仍处于登录状态。

如果第 5 步退出了登录，说明 CloakBrowser 服务器未遵守稳定的 `userId`。请检查配置路径，确认编辑 `config.yaml` 后已完全重启 Hermes，并验证您的 CloakBrowser 服务器版本是否支持基于用户的持久化 profile。

##### 状态存储位置

Hermes 从 profile 范围目录 `~/.hermes/browser_auth/cloakbrowser/`（非默认 profile 则在 `$HERMES_HOME` 下的对应位置）派生稳定的 `userId`。实际浏览器 profile 数据存储在 CloakBrowser 服务器端，以该 `userId` 为键。要完全重置持久化 profile，请在 CloakBrowser 服务器端清除对应数据，并删除相应 Hermes profile 的状态目录。

#### 外部管理的 CloakBrowser 会话

当另一个应用驱动可见的 CloakBrowser 浏览器（桌面助手、自定义集成、另一个 Agent）时，可配置 Hermes 在同一身份下运行，而非启动独立的隔离 profile。

三个参数控制行为：

| 设置 | 环境变量 | 效果 |
|---------|---------|--------|
| `browser.cloakbrowser.user_id` | `CLOAKBROWSER_USER_ID` | Hermes 创建标签页时使用的 CloakBrowser `userId`。设置此项即进入"外部管理"模式。 |
| `browser.cloakbrowser.session_key` | `CLOAKBROWSER_SESSION_KEY` | 创建标签页时发送的 `sessionKey`（即 `listItemId`）。用于接管时匹配已有标签页。未设置时默认为每任务值。 |
| `browser.cloakbrowser.adopt_existing_tab` | `CLOAKBROWSER_ADOPT_EXISTING_TAB` | 为 true 时，Hermes 在首次使用时调用 `GET /tabs?userId=<user_id>` 并优先复用已有标签页，而非新建。 |
...
接管仅在会话的 `tab_id` 填充之前触发一次。若外部应用在运行中关闭了被接管的标签页，下次浏览器工具调用将返回 CloakBrowser 错误 — Hermes 不会在每次调用时重新轮询新标签页。
...
**并发说明：** 外部应用和 Hermes 可同时驱动同一 CloakBrowser `userId`，但 CloakBrowser 不会在客户端之间协调每个标签页的焦点。请在应用层协调所有权（例如，Hermes 运行时外部应用暂停）。

#### VNC 实时查看

当 CloakBrowser 以有头模式运行（带可见浏览器窗口）时，其健康检查响应中会暴露 VNC 端口。Hermes 自动发现此信息，并在导航响应中包含 VNC URL，Agent 可分享链接供您实时查看浏览器。
...
**仅在会话启动时 CDP 端点可访问的情况下可用** — 即 `/browser connect` 已连接到运行中的 Chrome、Brave、Chromium 或 Edge 浏览器，或 `config.yaml` 中设置了 `browser.cdp_url`。默认本地 agent-browser 模式、CloakBrowser 和云端提供商（Browserbase、Browser Use、Firecrawl）目前不向此工具暴露 CDP — 云端提供商有每会话 CDP URL，但实时会话路由是后续功能。
...
| CloakBrowser / 默认本地 agent-browser | ✗ | ✗（无 CDP 端点） |
